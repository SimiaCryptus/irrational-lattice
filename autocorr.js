// Autocorrelation analysis for the lattice field.
  ...
  function fft2d(re, im, N, inverse) {
    ...
    for (let y = 0; y < N; y++) {
      for (let x = 0; x < N; x++) { row[x] = re[y * N + x]; rowI[x] = im[y * N + x]; }
      fft1d(row, rowI, inverse);
      for (let x = 0; x < N; x++) { re[y * N + x] = row[x]; im[y * N + x] = rowI[x]; }
    }
  ...
  export function topAutocorrVectors(data, size, zoom, maxN = 64) {
      ...
      for (let x = 1; x < N - 1; x++) {
        ...
        for (let oy = -1; oy <= 1 && isMax; oy++) {
          for (let ox = -1; ox <= 1; ox++) {
            ...
            if (ac[(y + oy) * N + (x + ox)] > v) { isMax = false; break; }
          }
        }
    ...
    for (const c of candidates) {
      ...
      for (const p of picked) {
        if (Math.hypot(c.dx - p.rdx, c.dy - p.rdy) < minR ||
            Math.hypot(c.dx + p.rdx, c.dy + p.rdy) < minR) { dup = true; break; }
      }
      if (dup) continue;
