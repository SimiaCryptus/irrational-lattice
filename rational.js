// Rational-arithmetic helpers for the irrational-lattice lab.
  ...
  export function rationalApprox(x, maxDen = 100) {
    ...
    let h0 = 0, h1 = 1; // numerator convergents
    let k0 = 1, k1 = 0; // denominator convergents
    let a = Math.floor(x);
    let frac = x - a;
    let bestNum = a, bestDen = 1;
    ...
    return { num: sign * bestNum, den: bestDen, value: (sign * bestNum) / bestDen };
  }
...
// --- Zoom slider -> continued fraction approximation ---
    ...
    function syncSliderFromFraction() {
      ...
      const lo = Number(zoomSlider.min), hi = Number(zoomSlider.max);
    ...
    return {
      getState: readState,
      setSize(v) { sizeInput.value = clampInt(v, 1, 4096); emit(); },
